...
include "koneksi.php";
session_start();
$idmusik = $_GET['id_musik'];
$qmusik = "SELECT * FROM musik";
$data_musik = $conn->query($qmusik)->fetch_all(MYSQLI_ASSOC);
$qMusikSpecific = "SELECT * FROM musik WHERE id_musik = ".$_GET['id_musik'];
$dataMusikSpecific = $conn->query($qMusikSpecific)->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="en">
<head>
  ...
  <div class="container">
    <div class="row">
        ...
        <div class="thumbnail">
          <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active" aria-current="page">Musik</a></li>
            </ol>
          </nav>
          <div> 
            <img src="uploads/<?php echo $dataMusikSpecific[0]['gambar'];?>" width="70%">
          </div>
          <div> 
            <audio control = true src="uploads/<?php echo $dataMusikSpecific[0]['audio'];?>" loop controls>
          </div>
            <div>
              <h2 class="my-0"><?php echo $dataMusikSpecific[0]['judul']; ?></h2>
              <small class="text-muted"><?php echo $dataMusikSpecific[0]['pencipta']; ?></small>
              <small class="text-muted"><?php echo $dataMusikSpecific[0]['lirik']; ?></small>
            </div>
          <div class="comment">
      ...
      <div class="col-sm-4">
        <div class="row">
          <div class="col-sm-12">
            <?php foreach($data_musik as $musik){?>
              <div class="caption">
                <div> 
                  <img src="uploads/<?php echo $musik['gambar'];?>" width="80px" height="80px">
                </div>
                <div>
                  <h4 class="my-0"><a class="text" href="Musikadm.php?id=<?php echo $musik['id_musik']?>"><?php echo $musik['judul'] ?></a></h4>
                  <p class="text-muted text-size"><?php echo $musik['lirik'] ?></p>
                </div>
              </div>
            <?php }; ?>
          </div>
